...
def get_at_content(dna): # define function get_at_content for variable 'dna'
    dna = dna.replace('N', '') 
    dna = dna.replace('n', '') #remove N/n characters 
    length = len(dna) # define length as length of dna
    a_count = dna.upper().count('A') # define a_count as occurances of A
    t_count = dna.upper().count('T') # define t_count as occurances of T
